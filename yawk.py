...
class YAWK:
    ...
    def _create_image(self):
        ...
        # Boxes positions
        # current condition
        CURRENT = {'x': 0, 'y': 0, 'w': int(2 * WIDTH / 3), 'h': int(HEIGHT / 3)}
        # today's forecast
        TODAY = {'x': CURRENT['w'], 'y': 0, 'w': WIDTH - CURRENT['w'], 'h': CURRENT['h']}
        # tomorrow
        TOMORROW = {'x': 0, 'y': CURRENT['h'], 'w': WIDTH, 'h': int((HEIGHT - CURRENT['h']) / 3)}
        # next 3 days
        NEXT_DAYS = [{'x': 0, 'y': CURRENT['h'] + TOMORROW['h'], 'w': int(WIDTH / 3), 'h': HEIGHT - CURRENT['h'] - TOMORROW['h']}]
        ...
        comfortaa = ImageFont.truetype("fonts/Comfortaa-Regular.ttf", 60)
        ...
        # temperature
        temp_font = ImageFont.truetype("fonts/Comfortaa-Regular.ttf", int(HEIGHT / 10))
        ...
        # celsius
        img.paste(celsius_icon.resize((celsius_icon.size[0] * 2, celsius_icon.size[1] * 2)), (BORDER * 3 + temp_w, int(2 * CURRENT['h'] / 7)))
        # condition icon
        ...
        x = int((CURRENT['w'] + temp_end_x) / 2 - condition.size[0] / 2)
        img.paste(condition, (x, int(CURRENT['h'] / 2 - condition.size[1] / 2)))
        ...
        x = int(BORDER + wind_icon.size[0] / 2 - humidity_icon.size[0] / 2)
        ...
        def print_temp(pos, text, temp, scale=1.0):
            ...
            # celsius
            x += temp_width
            img.paste(celsius_icon.resize((int(celsius_icon.size[0] * scale), int(celsius_icon.size[1] * scale))), (int(x), int(y)))
        ...
        y = int(3 * TODAY['h'] / 4 - condition.size[1] / 2)
        x = int(TODAY['x'] + TODAY['w'] / 2 - condition.size[0] / 2)
        ...
        # ip address
        ip_w, ip_h = draw.textsize(self.ip_address, font=tiny_font)
        draw.text((WIDTH - BORDER - ip_w, HEIGHT - BORDER - ip_h), self.ip_address, font=tiny_font, fill=gray)
